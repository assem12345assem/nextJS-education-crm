    ...
    try {
        switch (req.method){
            case "POST":
                ...
                if (role !== 'mentor') {
                    return res.status(400).json({ message: "Restrict, only mentor can apply rec. requests."})
                }
                ...
                if (request.status !== 'pending' )
                    return res.status(400).json({ message: "Request either accepted or declined already."})

                const recommendationLetter: RecommendationLetter = {
                ...
                await db.collection('recommendationLetters').insertOne(recommendationLetter)

                await db.collection(collectionName).updateOne({ _id: request._id }, { $set: { status: 'done' }} )
